...
NameCard::NameCard(const NameCard &ref) {
    int name_len = strlen(ref.name) + 1;
    this -> name = new char(name_len);
    strcpy(this -> name, ref.name);
    int company_name_len = strlen(ref.company_name) + 1;
    this -> company_name = new char(company_name_len);
    strcpy(this -> company_name, ref.company_name);
    int phone_number_len = strlen(ref.phone_number) + 1;
    this -> phone_number = new char(phone_number_len);
    strcpy(this -> phone_number, ref.phone_number);
    this -> pos = ref.pos;
