...
template<class T>
Heap<T>::Heap(int (*comp)(T data1, T data2)):numOfData(0),comp(comp) {
...
void Heap<T>::insert(T data) {
    ...
    while (idx != 1) {
        parent_idx = idx / 2;
        if (this -> comp(data, this -> arr[parent_idx].data) == 0) {
...
T Heap<T>::delete_() {
    ...
    while (1) {
        ...
        if (left_child_idx > this -> numOfData) {
            break;
